...
#[test]
fn edge_home_selection_uses_default_home_without_override() {
    let selected = select_edge_home(None, Some(OsString::from("/home/alice"))).unwrap();
    assert_eq!(selected.edge_home, PathBuf::from("/home/alice/.tenex-edge"));
    assert_eq!(
        selected.default_edge_home,
        Some(PathBuf::from("/home/alice/.tenex-edge"))
    );
    assert!(!selected.tenex_edge_home_set);
    assert!(selected.edge_home_is_default);
}
...
fn edge_home_selection_reports_explicit_non_default_home() {
    ...
    .unwrap();
    assert_eq!(selected.edge_home, PathBuf::from("/tmp/te-test-home"));
    assert_eq!(
        selected.default_edge_home,
        Some(PathBuf::from("/home/alice/.tenex-edge"))
    );
...
#[test]
fn edge_home_selection_refuses_absent_home_without_override() {
    assert_eq!(select_edge_home(None, None), Err(MISSING_HOME_MESSAGE));
}
