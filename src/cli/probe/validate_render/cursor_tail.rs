//! Cursor evidence renderer for `probe validate`.
...
pub(super) fn render(out: &mut String, evidence: &Value) {
    let _ = writeln!(out);
    let _ = writeln!(out, "cursor evidence");
    if !bool_at(evidence, "found") {
        ...
        let _ = writeln!(
            out,
            "  - graph cursor={} frame={} delta_since={}",
            int_at(evidence, "graph_cursor"),
            str_at(evidence, "graph_last_frame"),
            int_at(evidence, "graph_delta_since")
        );
        if bool_at(evidence, "session_row_found") {
            let _ = writeln!(
                out,
                "  - local seen_cursor={}",
                int_at(evidence, "local_seen_cursor")
            );
        }
    }
