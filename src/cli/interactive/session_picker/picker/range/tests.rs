...
use crate::{
    cli::interactive::session_picker::{
        data::SessionRow, picker::state::PickerState, SessionChoice,
    },
    session_state::SessionState,
};
use crossterm::event::{KeyCode, KeyEvent, KeyModifiers};

fn choice(handle: &str, running: bool, last_seen: u64) -> SessionChoice {
    SessionChoice {
        row: SessionRow {
            handle: handle.into(),
            running,
...
fn tab_is_not_a_history_range_control() {
    let mut state = PickerState::new(vec![choice("live", true, 0)]);
