...
use nmp::{
    ...
    AuthPolicyRequest, Binding, Demand, Engine, EngineConfig, IndexedTagName, LiveQuery, RelayUrl,
    SourceAuthority, Window, WriteStatus,
};
...
fn pinned_query(relay: RelayUrl, filter: nmp::Filter, access: AccessContext) -> Result<LiveQuery> {
...
fn nmp_filter(filter: Filter) -> Result<nmp::Filter> {
    ...
    Ok(nmp::Filter {
        ...
        authors: filter.authors.map(|authors| {
            Binding::Literal(authors.into_iter().map(|author| author.to_hex()).collect())
        }),
        kinds: filter
            .kinds
...
fn receive_window(subscription: nmp::Subscription, timeout: Duration) -> Result<Vec<Event>> {
