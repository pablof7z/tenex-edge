...
surfaces: agent UI, croissant logs, Nostr events, hook logs, and mosaico
daemon/relay logs.
...
not appear in mosaico logs. `nak` may show persisted events after the UI has
scrolled away.

## Inspecting Agent UI

For launch-mode runs, open the human session picker and choose Attach:

```bash
bash containers/mosaico/run --profile claude mosaico sessions
```

To drive the agent without attaching, send a direct channel mention:
