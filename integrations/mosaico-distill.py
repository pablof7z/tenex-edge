#!/usr/bin/env python3
"""mosaico activity distiller (LLM, gated).
...
`MOSAICO_DISTILL_CMD=python3 /path/to/mosaico-distill.py`.

Fast + cheap by design: tiny model, low max_tokens, short timeout. On ANY error
it prints nothing and exits 0, so mosaico falls back to its heuristic.

Key resolution (first hit wins):
  $MOSAICO_OPENROUTER_KEY
  ~/.proactive-context/config.json  -> openrouter_api_key
  ~/.local/share/opencode/auth.json -> openrouter.key
Model: $MOSAICO_DISTILL_MODEL (default: a cheap fast model).
"""
import json
import os
import sys
...
MODEL = os.environ.get("MOSAICO_DISTILL_MODEL", "openai/gpt-4o-mini")
SYSTEM = (
...
def api_key():
    k = os.environ.get("MOSAICO_OPENROUTER_KEY", "").strip()
    if k:
        return k
    try:
...
def main():
    ...
    try:
        ...
        return  # silent → mosaico uses its heuristic
