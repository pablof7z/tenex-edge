...
Agent list enumeration follows the same per-fabric pattern as project list: uniform shape, provider-owned source, with NIP-29 deriving the list from group membership, MLS from the group roster, and kind1 deriving it (derived/Option fallback) from kind:0 profiles + group membership plus a local whitelist, not from observed `agent`-tagged authors.
