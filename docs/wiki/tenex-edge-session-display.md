...
The session-start hook outputs a JSON object containing `session_id` and `short_code` fields, rather than a bare session ID string. <!-- [^ea5dd-3] -->

Session titles vary by harness: Codex exposes a native session title via the `thread_name` field in `~/.codex/session_index.jsonl` (keyed by session ID). OpenCode exposes a native session title via the `title: string` field on the `Session` type in `@opencode-ai/sdk`, and the plugin already has access to the live `Session` object. Claude Code provides no native title—`sessions-index.json` contains only `firstPrompt` (a raw truncated first message), and JSONL transcript entries contain no title field—so it must rely on LLM distillation from the transcript. <!-- [^68c8b-2] -->
