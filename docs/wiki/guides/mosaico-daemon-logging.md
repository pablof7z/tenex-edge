---
title: Mosaico Daemon Logging
slug: mosaico-daemon-logging
topic: mosaico
...
The log level strategy is: ERROR for unrecoverable errors, WARN for recoverable-unexpected conditions, INFO for consequential single events, DEBUG for high-frequency or expected events. The default `RUST_LOG` filter for the daemon is `mosaico=info`. <!-- [^47f3c-e88ec] -->
...
Relay event lines (`[→relay]`/`[relay✗]`) are routed through `tracing::debug!` so they only appear on stdout at `RUST_LOG=mosaico=debug` and are always written to relay.log. <!-- [^47f3c-b622e] -->
...
All previous `MOSAICO_DEBUG`-gated `eprintln!` calls in the daemon are replaced with structured `tracing` macros. <!-- [^47f3c-6f42f] -->
