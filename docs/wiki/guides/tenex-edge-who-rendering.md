...
`who` stays a pure human fabric view. Local live-session control belongs in the inline `mgmt session list` picker, while `who --expired` owns resumable dead or old sessions. Agent awareness belongs exclusively to `my session`.
