...
topic: agent-skills
summary: This guide governs the `tenex-edge` agent skill written to
tags:
  - capture
volatility: warm
confidence: medium
...
The `tenex-edge` skill is the sole usage authority for `tenex-edge`: it teaches day-to-day agent fabric usage — `my session`, channel read/send/wait, dispatch, membership, and channel navigation — and carries the rule to verify live help before trusting docs. Codex global memory must not retain `tenex-edge` usage or operational guidance; it should hold only codebase-internal engineering context.

The `tenex-edge` skill exposes two identity scopes: a stable agent identity (e.g. `codex`) and a session identity (e.g. `@nova-codex`), so independent skills can select the appropriate scope without coupling. Skills that manage agent-scoped state should prefer the stable agent identity and avoid using a session identity when an agent identity is available.

The `tenex-edge-dev` skill teaches hook wiring and debugging for Codex, Claude Code, and OpenCode, including how to prove agent registration with `my session`.
...
The `tenex-edge-docs-queue` skill teaches repository discipline: GitHub Issues are the tactical queue, no new planning files, correct docs in place, classify generated wiki output, and retire stale planning material.

The `tenex-edge` skill itself is resource-free: no `reference/` files or scripts are included, only the mental-model guidance and a small mechanics appendix.
