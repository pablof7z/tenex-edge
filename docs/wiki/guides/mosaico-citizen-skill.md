---
title: Mosaico Agent Skill
slug: mosaico-citizen-skill
topic: agent-skills
summary: This skill teaches the mental model for self-organizing on a mosaico fabric through shared awareness
tags:
  - capture
volatility: warm
confidence: medium
...
- `mosaico channel read` — read recent messages; pass `--channel` when joined to multiple channels
- `mosaico my session` — full self/fabric briefing (only when the hook snapshot is missing or stale)
- `mosaico channel send` — post a message; pass `--channel` when joined to multiple channels
- `mosaico channel send --tag <agent> --wait 600 --message "..."` — post a request and block for that target's correlated reply
- `mosaico wait 60 [--channel <path>]... [--from <member>]` — block for the next qualifying chat; no channel flags means every active channel
- `mosaico dispatch <agent[@backend]> --workspace <workspace> --message "..."` — start a delegated session in an explicit workspace
- `mosaico channel add --session @codex-quill-peak-369 <path>` — pull an existing session into a channel
- `mosaico channel add <pubkey|npub|nip05> <path> [--admin]` — add a human (optionally as admin)
- `mosaico channel list` — list available channels
- `mosaico channel switch <path>` — switch to an existing channel
- `mosaico channel create` — create a new channel <!-- [^019f1-5bca5] -->
