---
title: Mosaico Wait & Send
slug: mosaico-wait-send
topic: mosaico
summary: This guide covers the `wait` and `channel send --wait` primitives in mosaico
tags:
  - capture
volatility: warm
confidence: medium
...
This guide covers the `wait` and `channel send --wait` primitives in mosaico. Both commands share one agent-native output contract with no JSON mode, tables, or alternate formats. <!-- [^019f5-f94d6] -->
...
Success output for wait and `channel send --wait` uses the existing agent envelope format: `<mosaico>` containing `<channel ref="x">` with `<message from="@agent5" id="abc123">`. <!-- [^019f5-c547a] -->

Timeout output uses the agent-native envelope `<mosaico><wait outcome="timeout" after="60s" /></mosaico>. <!-- [^019f5-3eda8] -->
