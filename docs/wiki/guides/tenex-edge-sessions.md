---
title: Tenex-Edge Sessions Command
slug: tenex-edge-sessions
topic: tenex-edge
summary: The unified session management surface is the top-level command `tenex-edge sessions`
tags:
  - capture
volatility: warm
confidence: medium
...
# Tenex-Edge Sessions Command

## Command Surface

The unified session management surface is the top-level command `tenex-edge sessions`. The public `mgmt session` and `pty` command trees are removed entirely with no aliases. The `tenex-edge sessions` command is human-only; agent help hides it.

The `tenex-edge sessions` command opens an interactive fuzzy picker that shows all alive local sessions and allows selecting one to attach or kill.

Inject and resize internal mechanisms are retained for runtime use even though their public command verbs are removed from the session picker surface. <!-- [^019f5-2e38c] -->
