---
title: Mosaico Model Config
slug: mosaico-model-config
topic: mosaico
summary: `providers.json` and `llms.json` are config files living under `~/.mosaico` that drive model selection per role
tags:
  - capture
volatility: warm
confidence: medium
...
# Mosaico Model Config

## Model Configuration

`providers.json` and `llms.json` are config files living under `~/.mosaico` that drive model selection per role. A separate `config.json` holds additional manually-edited knobs: relays/indexerRelay, backendName, userNsec/mosaicoPrivateKey, ptyStatusCommand, perSessionRooms, and whitelistedPubkeys. The config store reads and writes `providers.json`/`llms.json` as raw JSON while preserving unknown keys. Writes to config files are atomic.

mosaico reads only the `mosaico-distillation` role from `llms.json`, and for each configuration entry it reads only the `provider` and `model` fields. The supported providers are `openrouter`, `ollama`, and `claude-cli`.

The `mosaico-distillation` role resolves to the `claude-cli` provider with model `claude-haiku-4-5-20251001`.

`llms.json` is pruned to contain only the `mosaico-distillation` role and the single `claude-haiku` configuration it points to, with no dead keys or fields. When pruning, a backup of the original file is saved at `~/.mosaico/llms.json.bak`.

<!-- citations: [^026d1-d15cd] [^dae38-a2ce1] [^026d1-deaa8] -->
## `mosaico mgmt config` Command

The `mosaico mgmt config` command is a TUI for configuring providers and models, built with the `inquire` crate. It was developed with a parallel Fable UX/design agent to produce a UX/UI spec covering palette, screen mockups, fuzzy-row anatomy, iconography, confirm-before-write pattern, and inquire RenderConfig mapping; that spec is stored at a scratchpad path as a 478-line markdown file.

The `mosaico mgmt config` menu offers two top-level options: configuring providers and configuring models. The only other TUI in the mosaico codebase (`pty_cli/tui_*.rs`) is the session-spawner picker, which is unrelated to model config.
...
The models menu flow is: role picker → provider picker (skipped if only one provider is configured) → fuzzy-searchable live model list → confirm → write. The role picker does not allow free-text entry of arbitrary new roles; only actually-used roles like `mosaico-distillation` are offered. Model configuration fetches live model listings from the configured provider endpoints (e.g., local Ollama, Ollama cloud, OpenRouter) using Ollama `GET /api/tags` and OpenRouter `GET /api/v1/models` (including context length and pricing) and presents them with fuzzy search.
...
The confirm-before-write screen shows a semantic summary (e.g., `mosaico-distillation → qwen2.5:7b (ollama) was: mistral:7b`), defaults to Yes, and answering No returns to the previous prompt with state intact.
