...
Chat mentions accept agent-instance labels (e.g. @haiku, @haiku1) that resolve to the instance's selected pubkey, replacing the former @<codename> targeting vocabulary. The `extract_mentions` tokenizer keeps tokens matching `[A-Za-z0-9._-]+` optionally host-qualified as `label@host`, instead of filtering to NATO-codename shape. `resolve_agent_pubkey(slug, host)` resolves an agent-instance label to the selected pubkey via the relay_profiles kind:0 store. The old `find_session_by_codename` lookup and the bail requiring a mention to name a concrete session codename are removed from `chat_write.rs`. <!-- [^bd868-1c088] -->

The chat-write confirmation line reads `mentioning @{label}`, driven by the RPC's `mentioned_label`, falling back to plain `sent chat {id}` when no mention is present. README.md chat-write documentation references `@<agent-label>` targeting instead of `@<codename>`. <!-- [^bd868-dce28] -->
## Ambient Chatter
...
The `who` command, when run inside an agent session, displays a self-identity header (label, channel, host, pubkey, status, member, pending) folding in `whoami`'s former role. The self-header reads `You are **{label}** on **{channel}** ({host}).` with pubkey, status, member, and pending info, and never shows a codename or raw session id. The codename disambiguation apparatus (`display_row_agent_name`, `agent_name_counts_for_scope`, `agent_count_key`) is deleted from `who` renderers; `WhoRow.slug` (carrying the ordinal label post-#98) renders directly. The `codename` field is dropped from `session_start` responses, hook `echo-session-id` responses, and `whoami`/`who` responses. `session_label` returns `agent@host` (degrading to raw `session_id` only when slug is empty), replacing the former codename `(agent@host)` format. <!-- [^bd868-e816c] -->
