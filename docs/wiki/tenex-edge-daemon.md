...
The daemon accept loop must start immediately after binding the socket, without waiting for relay connections to complete. Relay connection and warmup must be moved to a background task that runs concurrently after the accept loop starts. The daemon must bind the socket and open the store before constructing DaemonState and starting the accept loop, rather than binding the socket after relay connection completes. The transport and provider fields on DaemonState must be held as Mutex<Option<Arc<...>>> so they can be populated once the background relay connection succeeds. A relay_ready: Notify must fire once when the relay connects, allowing any handler that needs the relay to await readiness via state.transport().await or state.provider().await. The who and tmux_* RPCs must read exclusively from the local SQLite store so that the TUI renders immediately from last-known state without waiting for relay readiness. Relay-dependent operations (publish, subscribe, session start) must wait for relay readiness before executing.

<!-- citations: [^56f9f-1] [^162f9-5] [^98f99-26] [^412e3-1] [^412e3-2] -->
## Database
