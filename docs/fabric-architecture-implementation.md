...
- Existing host adapters stay thin and keep their current CLI/RPC surface:
  `who`, `chat read/write`, `project list/init/edit`, `channels
  create/list/join/leave/switch`, `agents`, `invite`, `harness hook`,
  `harness statusline`, `publish`, and `launch`.
...
1. `chat write` to a hosted sibling session delivers one local message using the
...
6. Manual smoke:
   ...
   - verify the target session receives the mention through its host hook or
     tmux delivery path;
